...
from app.users.users import get_cache_token, refresh_cache_token
...
#     violators = {}
#     winners = []
#     random = Random(current_app.config['RANDOM_API_KEY'])
...
def get_cached_url():
    ...
    if 'n' not in request.form:
        return {'error': 'Missing parameter: n.'}
    url = request.form['url']
    try:
        n = int(request.form['n'])
    except ValueError:
        return {'error': 'N parameter must be int.'}
    token = get_cache_token()
    if 'error' in token:
        return token, 401
    cached = {}
    response = requests.post(current_app.config['CACHE_URL'] + 'cache', data={'url': request.form['url']}, headers={'Authorization': 'Bearer ' + token})
    if response.status_code == 401:
        token = refresh_cache_token()
        response = requests.post(current_app.config['CACHE_URL'] + 'cache', data={'url': request.form['url']}, headers={'Authorization': 'Bearer ' + token})
    if response.status_code == 200:
        cached = json.loads(response.text)
